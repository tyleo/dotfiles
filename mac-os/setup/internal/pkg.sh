...
# App Store, or a language toolchain). Microsoft signs and notarizes the PowerShell
# pkg, so `installer` runs it with no Gatekeeper bypass. These tools do not self-
# update, so each version below is pinned and acts as the source of truth: bump the
# version and its matching URL together and the next run upgrades in place.
set -euo pipefail
...
install_pkg "PowerShell" "pwsh" "7.6.3" \
