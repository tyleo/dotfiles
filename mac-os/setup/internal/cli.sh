...
# Oh My Zsh custom plugins - the third-party plugins ~/.zshrc enables in plugins=()
# but Oh My Zsh does not bundle (git is bundled, these are not), so clone each into
# $ZSH_CUSTOM/plugins by hand. Runs after the Oh My Zsh install above, which creates
# that directory. $ZSH_CUSTOM is only exported once ~/.zshrc sources Oh My Zsh, so
# fall back to its default here. fzf-tab also needs the fzf binary, which the Brewfile
# installs. Each row is a "<name>" line then its "<url>" line; install_rows clones
# each. Entries are ABC-ordered by name.

# Clone one plugin into $ZSH_CUSTOM/plugins/<name>, unless it is already there. The
# functor for the install_rows loop below.
install_zsh_plugin() {
