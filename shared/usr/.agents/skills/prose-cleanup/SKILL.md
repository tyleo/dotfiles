---
name: prose-cleanup
description: Cleans up specified prose (markdown docs, a section, new text in a change, ...) to a terse, ASCII-only house style with no LLM tells.
disable-model-invocation: true
---

# Prose Cleanup

A manual style pass over prose. The invocation picks the target. Edit the text in place, like an auto-formatter; don't flag it for review.

## Scope

Only touch that target; leave the rest alone. Example invocations:
...
2. **ASCII only.** Replace smart quotes, ellipsis (…), arrows (→), and accented/emoji characters with ASCII (straight quotes, `...`, `->`). Find them with `grep -nP "[^\x00-\x7F]" <files>`. Do not apply this rule to non-prose content, e.g. program syntax or generated diagrams.
3. **No LLM tells.**
   ...
   4. No buried verbs ("X is Y" stating a rule); give the relation the verb that fits it best (sets, decides, limits, ...). Genuine identities stay
   ...
   10. No "name" for carrying or omitting a value ("the property names the leaf", "an error names the flag", "the record never names the file format"); use the verb the relation earns (references, points to, identifies, specifies, includes, reports, says which, ...), or state the absence plainly ("the record has no file format field"). Literal naming stays ("the `name:` key names the memory")
